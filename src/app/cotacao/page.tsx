...
import { MessageCircle, ArrowLeft, Shield, MapPin, Loader2 } from 'lucide-react'
...
export default function CotacaoPage() {
  ...
  useEffect(() => {
    // Inicializa o chat se não há steps
  ...
  const handleButtonClick = async (optionId: string, value: any, nextStep?: string) => {
    ...
    const next = nextStep || getNextStep(currentStepId, value, formData)
    if (next) {
      setTimeout(() => {
  ...
  const handleInputSubmit = () => {
    ...
    // Próximo step
    const next = getNextStep(currentStepId, processedInput, { ...formData, ...updates })
    if (next) {
      if (next === 'processing') {
        setTimeout(() => {
          setCurrentStepId(next)
  ...
  const handleConsentSubmit = () => {
    ...
    const next = getNextStep(currentStepId, true, formData)
    if (next) {
      setTimeout(() => {
        setCurrentStepId(next)
        simulateBotTyping(next)
  ...
  const requestLocation = async () => {
    try {
      ...
      // Aqui você faria reverse geocoding para obter cidade/estado
  ...
  const simulateBotTyping = (stepId: string) => {
    ...
    setTimeout(() => {
      ...
      addChatStep({
        id: `bot-${Date.now()}`,
        message: chatFlow[stepId].message,
        type: 'bot',
        stepIndex: chatSteps.length
      })
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500">
              Etapa {chatSteps.filter(s => s.type === 'user').length + 1}/6
            </div>
          </div>
        </div>
      </header>
