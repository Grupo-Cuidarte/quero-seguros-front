...
import { lifeFlow, getNextStep } from '@/lib/chat-flows'
...
export default function CotacaoVidaPage() {
  ...
  const currentStep = lifeFlow[currentStepId]

  useEffect(() => {
    reset()
    updateFormData({ insuranceType: InsuranceType.LIFE })
  ...
  const handleButtonClick = async (optionId: string, value: any, nextStep?: string) => {
    ...
    const next = nextStep || getNextStep(currentStepId, value, formData, lifeFlow)
    if (next) {
      setTimeout(() => {
  ...
  const handleInputSubmit = () => {
    ...
    if (currentStepId === 'welcome') {
      ...
      updates.personalData = { ...formData.personalData, income: parseFloat(processedInput) }
    ...
    const next = getNextStep(currentStepId, processedInput, { ...formData, ...updates }, lifeFlow)
    if (next) {
      if (next === 'processing') {
        setTimeout(() => {
          setCurrentStepId(next)
  ...
  const handleConsentSubmit = () => {
    ...
    const next = getNextStep(currentStepId, true, formData, lifeFlow)
    if (next) {
      setTimeout(() => {
        setCurrentStepId(next)
        simulateBotTyping(next)
  ...
  const simulateBotTyping = (stepId: string) => {
    ...
    setTimeout(() => {
      ...
      addChatStep({
        id: `bot-${Date.now()}`,
        message: lifeFlow[stepId].message,
        type: 'bot',
        stepIndex: chatSteps.length
      })
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold text-gray-900">Seguro de Vida</span>
            </div>
            <div className="text-sm text-gray-500">Etapa {chatSteps.filter(s => s.type === 'user').length + 1}/9</div>
          </div>
        </div>
      </header>
            ...
            <div className="space-y-4">
              {chatSteps.map((step) => (
                <div key={step.id} className={`flex ${step.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl ${step.type === 'user' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-900'}`}>
                    {step.type === 'bot' && (
                      <div className="flex items-center mb-2">
                        <Shield className="h-4 w-4 text-purple-600 mr-1" />
                        <span className="text-xs font-medium text-purple-600">Assistente Vida</span>
                      </div>
                    )}
              ...
              {currentStep?.type === 'buttons' && (
                <div className="grid grid-cols-1 gap-3 max-h-80 overflow-y-auto">
                  {currentStep.options?.map((option) => (
                    <button key={option.id} onClick={() => handleButtonClick(option.id, option.value, option.nextStep)} className="flex items-center justify-center px-4 py-3 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-colors">
                      {option.label}
                    </button>
                  ))}
                </div>
              ...
              {currentStep?.type === 'consent' && (
                <div className="space-y-4">
                  <div className="flex items-start space-x-3">
                    <input type="checkbox" id="lgpd-consent" checked={consentChecked} onChange={(e) => setConsentChecked(e.target.checked)} className="mt-1 h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" />
                    <label htmlFor="lgpd-consent" className="text-sm text-gray-700 leading-relaxed">{currentStep.consentText}</label>
                  </div>
                  <button onClick={handleConsentSubmit} disabled={!consentChecked} className="w-full px-4 py-3 bg-purple-600 text-white rounded-xl hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed">Concordo e Continuar</button>
                </div>
              )}
