...
export default function AuthModal({ isOpen, onClose, mode, onModeChange }: AuthModalProps) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'register') {
        // Mock registration - in production this would call supabase.auth.signUp
        console.log('Mock registration:', { email, password, name })
        alert('Verifique seu e-mail para ativar sua conta!')
      } else {
        // Mock login - in production this would call supabase.auth.signInWithPassword
  ...
  const handleGoogleAuth = async () => {
    setLoading(true)
    try {
      // Mock Google OAuth - in production this would call supabase.auth.signInWithOAuth
      console.log('Mock Google OAuth login')
      onClose()
    } catch (error: any) {
      setError(error.message)
    } finally {
